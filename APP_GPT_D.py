...
- Si la question de suivi fait référence à une réponse précédente, trouve la valeur ou le sujet dans l'historique et incorpore-la.
...
def get_or_create_vector_db(docs: List[Document], index_path: str = VECTOR_INDEX_PATH):
    ...
    if os.path.exists(index_path) and os.path.exists(os.path.join(index_path, "index.faiss")):
        try:
            ...
            print(f"⚠️ Échec chargement, on recrée: {e}")
            
    if not docs: 
        print("❌ Aucun document disponible pour créer l'index.")
        return None
    ...
    try: vectorstore.save_local(index_path)
    except: pass
    return vectorstore
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        return JSONResponse({"status": "error", "message": f"Erreur écriture: {str(e)}"})

    new_docs = []
    if filename.lower().endswith(".pdf"):
        new_docs = extract_docs_from_pdf(file_path)
    elif filename.lower().endswith(".docx"):
        new_docs = extract_docs_from_docx(file_path)
    else:
    ...
    if vectorstore_global:
        try:
            ...
            print(f"✅ Ajouté {len(chunks)} chunks pour {filename}")
            return JSONResponse({"status": "success", "message": f"Fichier '{filename}' analysé et ajouté."})
        except Exception as e:
        ...
        try:
            ...
            return JSONResponse({"status": "error", "message": f"Erreur init: {str(e)}"})
